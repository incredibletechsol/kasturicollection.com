...
//    private $DEF_settings;
    ...
    public function getAttributes($DEF_settings, $data = array())
    {
        ...
        if (isset($data['category_id']) && $data['category_id']) {
            $sql .= " AND p2c.category_id IN
                    (SELECT category_id FROM " . DB_PREFIX . "category_path WHERE path_id = " . (int)$data['category_id'] . ")";
        }
        if (isset($data['special']) && $data['special']) {
    ...
    public function getOCFilters($DEF_settings, $data)
    {
        $sql = "SELECT f.*, fd.name, fgd.name as `group`
        ...
        switch ($DEF_settings['advance']['ocfilters']['sort']) {
            case 'sort':
                $sql .= "fg.sort_order, f.sort_order, fd.name, f.filter_id";
                break;
            case 'name':
                $sql .= "fgd.name, fd.name, f.sort_order, f.filter_id";
                break;
            default:
                $sql .= "f.filter_id, fd.name, f.sort_order";
                break;
        }
        ...
        if ($query->num_rows) {
            foreach ($query->rows as $row) {
                $output[$row['filter_group_id']]['name'] = $row['group'];
                $output[$row['filter_group_id']]['group_id'] = $row['filter_group_id'];
                $output[$row['filter_group_id']]['filter_values'][$row['filter_id']] = [
...
//                    $sql .= " AND text IN (" . implode(',', $attribute_value_ids) . ")";
    ...
    private function generalizeProducts($data)
    {
        ...
        //OCFILTER JOIN BEGIN
        if ($data['ocfilter']) {
            foreach ($data['ocfilter'] as $key => $filter_group) {
                $sql .= " LEFT JOIN " . DB_PREFIX . "product_filter pf" . $key . " ON (p.product_id = pf" . $key . ".product_id)";
            }
        }
        //OCFILTER JOIN END
        ...
        //OCFILTER WHERE BEGIN
        if ($data['ocfilter']) {
            foreach ($data['ocfilter'] as $key => $filter_group) {
                $filter_id = [];
                foreach ($filter_group as $option_value_id) {
                    if ($option_value_id) {
                        $filter_id[] = (int)$option_value_id;
                    }
                }

                if ($filter_id) {
                    $sql .= " AND pf" . $key . ".filter_id IN (" . implode(',', $filter_id) . ")";
                }
            }
        }
        //OCFILTER WHERE END
