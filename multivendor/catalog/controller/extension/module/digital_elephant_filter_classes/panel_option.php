<?php
Class ControllerExtensionModuleDigitalElephantFilterClassesPanelOption extends Controller
{
    /**
    ...
    public function getSectionGroups($setting, $category_id)
    {
        ...
        if (isset($setting['DEF_settings']['options']) && ($this->helperUrl->isAjaxRequest() || !$section_groups || !isset($setting['DEF_settings']['cache']['isset']))) {
            $args = array(
                'category_id' => $category_id,
                'filter_sub_category' => true
            );
            $options = $this->model_extension_module_digital_elephant_filter->getOptions($setting['DEF_settings'], $args);
            ...
            foreach ($options as $option) {
                ...
                $values = array();
                //if (!isset($options_type_by_setting[$option_id]['hide']) && count($option['option_values']) > 1) {
                if (!isset($options_type_by_setting[$option_id]['hide'])) {
                    ...
                    foreach ($option['option_values'] as $option_value) {
                        ...
                        if (isset($setting['DEF_settings']['is_display_total'])) {
                            ...
                            $input_label = $option_value['name'] . ' (' . $totalProducts . ')';
                        ...
                        $values[] = array(
                            ...
                            'image' => $image,
                            'is_enable' => $is_enable,
                            'is_active' => (isset($currentUrlData['option'][$option_id]) && in_array($option_value['option_value_id'], $currentUrlData['option'][$option_id]))
                        );
                    }
