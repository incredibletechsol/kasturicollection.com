...
class ControllerExtensionModuleDigitalElephantFilterClassesSeo extends Controller
{
    /**
        ...
        if (!empty($this->request->get['option'])) {
            foreach ($this->request->get['option'] as $option_ids) {
                foreach ($option_ids as $option_id) {
                    $model_option = $this->model_extension_module_digital_elephant_filter->getOptionValueDescriptions($option_id);
