<?php
Class ControllerExtensionModuleDigitalElephantFilterClassesPanelAttribute extends Controller
{
    /**
    ...
    public function getSectionGroups($setting, $category_id)
    {
        ...
        if (isset($setting['DEF_settings']['attributes']) && ($this->helperUrl->isAjaxRequest() || !$section_groups || !isset($setting['DEF_settings']['cache']['isset']))) {
            $args = array(
                'category_id' => $category_id,
                'filter_sub_category' => true
            );
            $attributes_groups = $this->model_extension_module_digital_elephant_filter->getAttributes($setting['DEF_settings'], $args);
            ...
            foreach ($attributes_groups as $attribute_groups_id => $attribute_group) {
                ...
                foreach ($attribute_group['attribute_values'] as $attribute_value_id => $attribute_value) {
                    ...
                    //if (!isset($attributes_type_by_setting[$attribute_value_id]['hide']) && count($attribute_value['values']) > 1) {
                        ...
                        foreach ($attribute_value['values'] as $value) {
                            if (!empty($value) || $value === '0') {
                                ...
                                if (isset($setting['DEF_settings']['is_display_total'])) {
                                    ...
                                    $input_label = $value . ' (' . $totalProducts . ')';
                                ...
                                $values[] = array(
                                    ...
                                    'image' => '',
                                    'is_enable' => $is_enable,
                                    'is_active' => (isset($currentUrlData['attribute'][$attribute_value_id]) && in_array($value, $currentUrlData['attribute'][$attribute_value_id]))
                                );
                            }
                        }
