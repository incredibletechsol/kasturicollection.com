...
if($this->config->get('module_maximum_status')){
             $product_maximum = $this->db->query("SELECT * FROM " . DB_PREFIX . "maximum   WHERE product_id = '" . (int)$product_query->row['product_id']. "' ");
			
			if($product_maximum->num_rows){
				$product_maximum_value = $product_maximum->row['maximum'];
			}else {
				$product_maximum_value = 0 ;
				}}
			else {
				$product_maximum_value = 0 ;
				}			
 ...
 'qnty'            => $product_query->row['quantity'], 
