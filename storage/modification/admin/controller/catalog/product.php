...
$this->load->language('catalog/product_maximum');
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			if (is_file(DIR_IMAGE . $result['image'])) {
				...
				$image_zoom = HTTP_CATALOG . 'image/' . $result['image'];
			} else {
				$image = $this->model_tool_image->resize('no_image.png', 40, 40);

				$image_zoom = HTTP_CATALOG . 'image/' . 'no_image.png';
			}
...
if (isset($this->request->post['maximum'])) {
				...
				$data['maximum'] = $this->model_extension_maximum->getMaximum($product_info['product_id']);
			  } else { 
			  $data['maximum'] = 0;
			  }
