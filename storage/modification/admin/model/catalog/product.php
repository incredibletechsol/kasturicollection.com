...
if (isset($data['maximum'])){ 
				 $this->db->query("INSERT INTO " . DB_PREFIX . "maximum SET product_id = '" . (int)$product_id . "',maximum ='". (int)$data['maximum'] ."' "); } 	
...
if (isset($data['maximum'])){
			$this->load->model('extension/maximum');
			$data['product_id'] = $this->model_extension_maximum->getProductid($product_id);
			if($data['product_id'] ==""){
			        $this->db->query("INSERT INTO " . DB_PREFIX . "maximum SET product_id = '" . (int)$product_id . "',maximum ='". (int)$data['maximum'] ."' ");
			        } else {
			        $this->db->query("UPDATE " . DB_PREFIX . "maximum SET maximum ='". (int)$data['maximum'] ."' WHERE product_id = '" . (int)$product_id . "' ");
			        }
			        }
...
$data['maximum'] = $this->getProductMaximum($product_id);
  ...
  public function getProductMaximum($product_id) {
					$query = $this->db->query("SELECT maximum FROM " . DB_PREFIX . "maximum WHERE product_id = '" . (int)$product_id . "'");

					if(isset($query->row['maximum'])) {
