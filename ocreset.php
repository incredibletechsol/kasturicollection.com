...
// Connect to database
$con = mysqli_connect(DB_HOSTNAME, DB_USERNAME, DB_PASSWORD,DB_DATABASE) or die('ERROR CONNECTING TO SERVER');
...
if(isset($_POST['ID'])) {
    ...
    if($pass == '') {
        ...
        $query = sprintf("UPDATE ".DB_PREFIX."user SET password = '%s' WHERE user_id = '%s'", md5($pass), mysqli_real_escape_string($con,$_POST['ID']));
        $result = mysqli_query($con,$query);
        if(!$result) {
