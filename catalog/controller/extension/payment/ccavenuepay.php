...
$file=DOM_BZ_PATH_PG_MAIN_201."main.php";// need to be edit for every ecommerce path
...
class ControllerExtensionPaymentCcavenuepay extends Controller {
	public function index() {
		...
		if ($order_info)		{
			$currency =  $b->getAllowedCurrency($order_info['currency_code']);
			if($currency==false)
			{
            ...
            if(!$this->config->get('payment_ccavenuepay_testmode'))
            {
                $k=1;
			...
			$data['action'] 							= $b->getbzmethod($k);
				
			$b->setBZCCLicenceApiTNPrefix(DB_PREFIX);
			$apidetails = $this->db->query($b->getPgmDetails())->rows; // passed instance of db variable
			...
			$api_resonse = $b->getfrontform($apidetails,json_encode($passdata));

			## The API response
			$data['api_resonse'] = $api_resonse;
			
			
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'extension/payment/ccavenuepay')) {
			 
				return $this->load->view($this->config->get('config_template') . 'extension/payment/ccavenuepay',$data);
			} else {
	...
	public function callback() 
	{
		...
		$rcvdString		= $b->decrypt($encResponse,$workingKey);	//Crypto Decryption used as per the specified working key.
		...
		if($order_info)
		{
			...
			{				
				...
				$payment_confirmation_mail =  $this->config->get('payment_ccavenuepay_payment_confirmation_mail');
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'extension/payment/ccavenuepay_response')) {
					$template = $this->config->get('config_template') . 'extension/payment/ccavenuepay_response';
				} else {
