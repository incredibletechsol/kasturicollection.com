<?php
Class ControllerExtensionModuleDigitalElephantFilterClassesPanelManufacturer extends Controller
{
    /**
    ...
    public function getSectionGroups($setting, $category_id)
    {
        ...
        if (($this->helperUrl->isAjaxRequest() || !$section_groups || !isset($setting['DEF_settings']['cache']['isset'])) && !isset($manufacturers_setting['hide'])) {
            ...
            $manufacturers = $this->model_extension_module_digital_elephant_filter->getManufacturers($setting['DEF_settings'], $args);
            $section_groups = [];
            $sections = [];
            ...
            //if (count($manufacturers) > 1) {
                ...
                foreach ($manufacturers as $manufacturer) {
                    ...
                    if (isset($setting['DEF_settings']['is_display_total'])) {
                        ...
                        $input_label = $manufacturer['name'] . ' (' . $totalProducts . ')';
                    ...
                    $values[] = array(
                        ...
                        'image' => $image,
                        'is_enable' => $is_enable,
                        'is_active' => (isset($currentUrlData['manufacturers']['manufacturer']) && in_array($manufacturer['manufacturer_id'], $currentUrlData['manufacturers']['manufacturer']))
                    );
                }
