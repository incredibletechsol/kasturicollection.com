<?php
Class ControllerExtensionModuleDigitalElephantFilterClassesPanelCategory extends Controller
{
    /**
    ...
    public function getSectionGroups($setting, $category_id)
    {
        ...
        if (($this->helperUrl->isAjaxRequest() || !$section_groups  || !isset($setting['DEF_settings']['cache']['isset'])) && !isset($categories_setting['hide'])) {
            $categories = $this->model_extension_module_digital_elephant_filter->getSubCategories($category_id, $setting['DEF_settings']);
            ...
            //if (count($categories) > 1) {
                ...
                foreach ($categories as $category) {
                    ...
                    if (isset($setting['DEF_settings']['is_display_total'])) {
                        ...
                        $input_label = $category['name'] . ' (' . $totalProducts . ')';
                    ...
                    $values[] = array(
                        ...
                        'image' => $image,
                        'is_enable' => $is_enable,
                        'is_active' => (isset($currentUrlData['category']['categories']) && in_array($category['category_id'], $currentUrlData['category']['categories']))
                    );
                }
